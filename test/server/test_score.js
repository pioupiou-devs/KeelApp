const {calculFrame,calculScoreTotal,calculLastFrame}=require("../../src/server/models/grid");
...
describe('calculFrame', function(){
  it('Calcul the score of a frame with no spare and no strike',function(){
    ...
    assert.equal(g.calculFrame("player1",1),3); // No strike, no spare
  });
  it('Calcul the score of a frame with one strike in a row',function(){
    ...
    assert.equal(g.calculFrame('player1',7),20); //1 strike in a row
  });
  it('Calcul the score of a frame with spare',function(){
    ...
    assert.equal(g.calculFrame('player1',3),15); //spare
  });
  it('Calcul the score of a frame with 3 strike in a row',function(){
    ...
    assert.equal(g.calculFrame('player1',5),30); // 3 strikes in a row
});
it('Calcul the score of a frame with 2 strike in a row',function(){
  ...
  g.players["player1"] = [
    ...
    assert.equal(g.calculFrame('player1',6),22); // 2 strikes in a row
  });
  it('Calcul the score of a frame with strike at the 9th frame',function(){
    ...
    assert.equal(g.calculFrame('player1',9),25);// strike at the 9th frame with 2 strikes in a row
  ...
  it('Calcul the score of the 10th frame',function(){
    ...
    assert.equal(g.calculFrame('player1',10),18);// case of the 10th frame
  });
  ...
  describe('calculScoreTotal', function(){
    ...
    it('Calcul the final score and the score accumulated of all the frames',function(){
      assert.equal(g.calculScoreTotal("player1"),168);
    }
